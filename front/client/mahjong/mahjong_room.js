$(function(){
  ...
  function _ui(n){ return $('#root #'+_class+(n?" "+n:"")); }
  ...
  function redraw(u, v){
    // get my sit
    var f = undefined;
    $.each(F, function(i,x){ if(v.sits[x].id == u){ f = x; return false; } });
    // clean bind
    _ui(".cmd").unbind();
    // redraw
    _ui().hide().empty();
  ...
  function _draw_play(f, v){
    ...
    $.each(F, function(i, x){
      ...
      s += "<ul id='"+xf+"'>";
      // show
      $.each(v.game[x].show, function(ii, y){
	s += _mahjong(xf, 2, y, "o");
      });
      s += _spacer();
      // hide
      $.each(v.game[x].hide, function(ii, y){
	s += _mahjong(xf, 1, y, "h");
      });
      s += _spacer();
      // hand
      $.each(v.game[x].hand, function(ii, y){
	if(xf == "S" && f == v.game.turn && v.game.zhua){
	  // 到我抓牌，手牌可弃 h active cmd
	  s += _mahjong(xf, 1, y, "h active cmd");
	}else{
	  // 非抓牌 或 未轮到我做决定，手牌不可弃，仅显示
	  // 我不在游戏中，将南面的手牌显示为未开
	  var st = (f == undefined && xf == "s") ? 0 : 1;
	  s += _mahjong(xf, st, y, "h");
	}
      });
      s += _spacer();
      // ting
      if (v.game[x].ting) s += _ting(xf);
      s += "</ul>";
      $.log(s);
      _ui("#side").append(s);
    });
    ...
    // 到我抓牌，手牌可弃
    if(f == v.game.turn && v.game.zhua){
      _ui("#side #S li.h.active.cmd").click(
	function(){ _pai("drop", $(this).attr("mj"), false); }
      );
    }
	...
	$.each(x.option, function(i, y){
	  ...
	  _ui("#cmds li:last").click(
	    ...
	    function(){
	      _ui("#side #S li.h[mj='"+y[0]+"']:first").css("margin-top", "-5px");
	      _ui("#side #S li.h[mj='"+y[1]+"']:first").css("margin-top", "-5px");
	    }
	  ).mouseout(
	    function(){
	      _ui("#side #S li.h[mj='"+y[0]+"']:first").css("margin-top", "");
	      _ui("#side #S li.h[mj='"+y[1]+"']:first").css("margin-top", "");
	    }
	  );
	});
      }
  ...
  $(document).bind(_class, function(event, instance){
    ...
    $.thread(function(){ _cast("enter_debug")(); }, 100); // enable debug mode
    $.thread(function(){ _cast('enter')(); }, 200);
    $.thread(function(){ _cast('refresh')(); }, 300);
...
/*
    ...
    $.thread(function(){ refresh('ue', view); }, 500);
*/
  });
